 ...
 */
public class BluetoothGpsBrowser extends List implements CommandListener,
		BluetoothHandler, Runnable {
	 ...
	 */
	public BluetoothGpsBrowser(final String title,
			final LocationSourceWaiter sourceWaiter,
	...
	public void commandAction(final Command cmd, final Displayable d) {
		if (cmd == back) {
			...
			final BluetoothProvider bluetoothProvider = new BluetoothProvider(
					(LocationDataConnectionProvider) device, found[index]
							.getUrl());
