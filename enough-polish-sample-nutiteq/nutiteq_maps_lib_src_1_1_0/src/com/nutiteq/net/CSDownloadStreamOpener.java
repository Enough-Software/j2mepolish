...
public class CSDownloadStreamOpener implements DownloadStreamOpener {
   ...
   */
  public CSDownloadStreamOpener(final String urlExtension) {
  ...
  public CSDownloadStreamOpener(final long timeout) {
    this("", timeout);
  }

  public CSDownloadStreamOpener(final String urlExtension, final long timeout) {
  ...
  private void openInputStream(final DownloadStreamWaiter streamWaiter, final String url,
    ...
    try {
      ...
      connection = (HttpConnection) Connector.open(downloadableUrl);
      connection.setRequestMethod(downloadable == null ? HttpConnection.GET : HttpConnection.POST);
      ...
      if (downloadable != null) {
        // TODO use stringToByteArray, convert 16-bit char to byte using casting
    ...
    if (redirects == MAX_FOLLOWED_REDIRECTS && redirectUrl != null) {
      streamWaiter.error(TOO_MANY_REDIRECTS, "Too many redirects created!");
      return;
    }
