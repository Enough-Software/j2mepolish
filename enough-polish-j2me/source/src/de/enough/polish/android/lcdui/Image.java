...
//		Bitmap bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
	...
	{
		InputStream stream = ResourcesHelper.getResourceAsStream(name);
		if (stream == null) {
			throw new IOException("for " + name);
		}
	...
	{
		Bitmap sourceBitmap = image.getBitmap();
		Bitmap subBitmap = Bitmap.createBitmap(sourceBitmap, x, y, width, height);
		Image resultImage = new Image(subBitmap);
		return resultImage;
	}
